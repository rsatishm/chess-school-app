...
export default class XChessground extends React.Component {
  ...
  buildConfigFromProps(props) {
    const config = { events: {} }
    Object.keys(XChessground.propTypes).forEach(k => {
      const v = props[k]
  ...
  componentDidMount() {
    console.log("XChessground create, props: " + JSON.stringify(this.buildConfigFromProps(this.props)))
    this.cg = NativeChessground(this.el, this.buildConfigFromProps(this.props))
    console.log(this.el.ownerDocument.body.innerHTML)
  }
